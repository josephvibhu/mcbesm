...
source "$SOURCE_DIR/src/mcbeup.sh"
source "$SOURCE_DIR/src/mcbebak.sh"
source "$SOURCE_DIR/src/mcbeinstall.sh"

mcbe() {
    # Check if the server directory exists
    if [ -d $SOURCE_DIR/server/bedrock-server ]; then
    ...
    worlds=$(find $SOURCE_DIR/server/bedrock-server/worlds/ -mindepth 1 -maxdepth 1 -type d | sed 's|^.*/||')
    ...
    sed -i "s/^level-name=.*/level-name=$world_name/" $SERVER_DIR/server.properties || { echo "Failed to update server.properties"; exit 1; }
